...
impl Instruction {
    ...
    pub fn opcode(&self) -> u8 {
        ...
        match self {
            ...
            MovR2RP(src, _) => case!(src, 0x0C),
            MovR2IP(src, _) => case!(src, 0x0E),
            MovRP2R(_, dest) => case!(dest, 0x10),
            MovRP2RP(_, _) => 0x12,
            MovRP2IP(_, _) => 0x13,
        }
    }
...
mod test {
    ...
    test_case!(
        movr2rp,
        Register::Rb0, Register::R1, [0x0C, 0x10];
        Register::R0, Register::R1, [0x0D, 0x10]
    );
    test_case!(
        movr2ip,
        Register::Rb0, Immediate::word(0x600D), [0x0E, 0x00, 0x0D, 0x60];
        Register::R0, Immediate::word(0x600D), [0x0F, 0x00, 0x0D, 0x60]
    );
    test_case!(
        movrp2r,
        Register::R0, Register::Rb1, [0x10, 0x10];
        Register::R0, Register::R1, [0x11, 0x10]
    );
    test_case!(
        movrp2rp,
    ...
    test_case!(
        movrp2ip,
        Register::R0, Immediate::word(0x600D), [0x13, 0x00, 0x0D, 0x60]
    );
}
